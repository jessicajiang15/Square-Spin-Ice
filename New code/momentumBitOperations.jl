...
function circular_shift(x::Integer, N::Integer, i::Integer)
    # Mask to isolate the first N bits
    mask = (1 << N) - 1
    # Isolate the first N bits of x
    relevant_bits = x & mask
    i = i % N

    # Circular shift
    if i >= 0
        # Left circular shift within the first N bits
        shifted = ((relevant_bits << i) | (relevant_bits >> (N - i))) & mask
    else
        # Right circular shift within the first N bits
        shifted = ((relevant_bits >> -i) | (relevant_bits << (N + i))) & mask
    end

    # Clear the first N bits in the original number and place the shifted bits
    result = (x & ~mask) | shifted
