...
function calculateEigensystemTransverseNoSymmetry(N, J, J2, hs::Vector{Float64},hs2::Vector{Float64}, bonds,eigmethod, num, h1orh2)
    ...
    eigensystem::Array{Any}=Any[];
    eigenvalues::Array{Any}=Any[];
    eigenvectors::Array{Complex}=Complex[];
    local Htemp;
    push!(theInfo, 0:2^(N)-1);
    println("timing h");
    @time begin
    ...
    elseif(eigmethod=="lanczos")
        ...
        eigtemp=LinearAlgebra.eigs((Htemp), nev=n, which="SR");
        append!(eigenvalues, eigtemp[1]);
        append!(eigenvectors, eigtemp[2]);
    end
...
#constructTransverseHamiltonianNoSymmetrySxSz
function calculateEigensystemTransverseNoSymmetrySxSz(N, J, J2, hx, hz, bonds,eigmethod, num)
    #println(randomList);
    #the info contains the states
    theInfo::Array{Any}=Any[];
    eigensystem::Array{Any}=Any[];
    ...
    local Htemp;
    push!(theInfo, 0:2^(N)-1);
    Htemp=constructTransverseHamiltonianNoSymmetrySxSz(bonds, N, J, J2, eigmethod, hx, hz)
    n::Int=0;
    if(num=="all")
        n=2^N;
    elseif(num=="one")
